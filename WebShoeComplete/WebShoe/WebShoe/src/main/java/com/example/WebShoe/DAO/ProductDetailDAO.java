...
public class ProductDetailDAO extends DAO{
	...
	public List<ProductDetail> selectProductDetails(int id){
		...
		try (Connection con = getConnection();
				PreparedStatement pr = con.prepareStatement("select tblproductdetail.id as id, tblproductdetail.quantity as quantity, "
						+ "tblproductdetail.size as size, tblproduct.name as name,tblproduct.imageMain, product_id "
			...
			while (rs.next()) {
				...
				u.getProduct().setImageMain(rs.getString("imageMain"));
	...
	public int getQuantityByID(int id) {
		int quantity = 0;
		try(Connection conn = getConnection(); 
				PreparedStatement ps = conn.prepareStatement("select quantity from tblproductdetail where id = ?");){
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
