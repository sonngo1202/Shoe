	...
	public List<ProductStat> selectProStats(Date st, Date et){
		...
		try (Connection con = getConnection();
				PreparedStatement ps = con.prepareStatement("select tblproduct.id as id, tblproduct.name as name, tblcategory.name as category, "
						+ "sum(tblitembill.amount) as total, sum(tblitembill.price*tblitembill.amount) + sum(distinct tblbill.fee) as revenue "
						...
						+ "group by tblproduct.id order by revenue desc")){
			ps.setDate(1, st);
			ps.setDate(2, et);
