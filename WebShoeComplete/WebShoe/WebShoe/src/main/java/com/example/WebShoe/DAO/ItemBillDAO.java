...
public class ItemBillDAO extends DAO{
	...
	private static final String SELECT_ITEMBILL_BY_BILLID = "select tblproduct.imageMain, tblproduct.name, tblproductdetail.size, tblitembill.amount, tblitembill.price \r\n"
    ...
    public List<ItemBill> getItemBillsByBillID(int idB){
    	...
    	try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_ITEMBILL_BY_BILLID)){
    		...
    		while(rs.next()) {
    			...
    			itemBill.setPrice(rs.getInt("price"));
