...
public class BillDAO extends DAO{
	...
	public List<Bill> selectBillsByDay(Date day){
		...
		try (Connection con = getConnection();
				PreparedStatement ps1 = con.prepareStatement("select tblbill.id as id, tbluser.name as name, sum(tblitembill.amount) as total, sum(tblitembill.price) + sum(distinct tblbill.fee) as revenue, tblbill.paymentTime as day "
						...
						+ "group by tblbill.id order by day");
	...
	public List<Bill> selectBillsByProduct(int idp, Date st, Date et){
		List<Bill> bills = new ArrayList<>();
		try (Connection con = getConnection();
				PreparedStatement ps1 = con.prepareStatement("select tblbill.id as id, tbluser.name as name, sum(tblitembill.amount) as total, sum(tblitembill.price*tblitembill.amount) + sum(distinct tblbill.fee) as revenue, tblbill.paymentTime as day "
						...
						+ "group by tblbill.id order by day");
				PreparedStatement ps2 = con.prepareStatement("select tblbill.id as id, tblcustomer.name as name, sum(tblitembill.amount) as total, sum(tblitembill.price*tblitembill.amount) + sum(distinct tblbill.fee) as revenue, tblbill.paymentTime as day "
